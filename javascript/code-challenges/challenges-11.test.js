...
function transformToLis(ele){
  // Solution code here...
  let key = Object.keys(ele).map(key => {
    return `<li>${key}: ${ele[key]}</li>`;
  });
  return key;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArray = input.map(item => item.filter(element => (typeof(element) === 'number') && element%5===0));
  return newArray.map(item => item.map(element => Math.pow(2, element)));
};
...
let findShortest = (data) => {
  ...
  let result = data
    ...
    .reduce((x, y) => (y.height > x.height ? y : x));
  return result.name;
};
