...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    secondDiagonal.push(board[i][board.length - 1 - i])
    firstArr.push(tic);
  }
  mainArr = board.concat(firstArr, [firstDiagonal], [secondDiagonal]);
  let result = false;
  for (let k = 0; k < mainArr.length; k++) {
    if (ifEqual(mainArr[k])) {
      result = true;
    }
    //mainArr-> [[x,o,x],[x,'',o],[x,x,o],[o,o,o],...]
  }
  return result;
};
